$(document).ready(function () {
  ...
  if (window.location.href.indexOf('schedule') > -1) {
    ...
    function createCards(events) {
      const cards = events.map((event) => createEl('div', { class: 'card-body clickable', onClick: () => onEventClick(event) }, createEl('h5', { class: 'card-title' }, event.title || ''), createEl('p', { class: 'card-text' }, event.description || createLoremIpsum()), createEl('hr')));
      return cards;
    }

    const containerEl1 = createEl('div', { class: 'container mt-5' }, createEl('div', { class: 'card mb-5' }, createEl('h5', { class: 'card-header' }, 'Day 1'), ...createCards(events.slice(0, 3))));

    const containerEl2 = createEl('div', { class: 'container' }, createEl('div', { class: 'card mb-5' }, createEl('h5', { class: 'card-header' }, 'Day 2'), ...createCards(events.slice(3, 6))));

    const containerEl3 = createEl('div', { class: 'container' }, createEl('div', { class: 'card mb-5' }, createEl('h5', { class: 'card-header' }, 'Day 3'), ...createCards(events.slice(6, 9))));
  ...
  if (window.location.href.indexOf('event') > -1) {
    ...
    const containerEl = createEl(
      ...
      createEl(
        ...
        createEl('div', { class: 'card-body' }, createEl('h1', { class: 'card-title' }, currentEvent.title || ''), createEl('h2', { class: 'text-muted' }, currentEvent.subtitle || ''), createEl('p', { class: 'card-text mt-3' }, currentEvent.description || createLoremIpsum(100)), createEl('a', { class: 'btn btn-primary', href: 'tickets.html' }, 'Buy Tickets'))
      )
    );
  ...
  if (window.location.href.indexOf('tickets') > -1) {
    ...
    function purchaseTicket() {
      ...
      modalEl.append(createEl('div', { class: 'modal-body' }, createEl('h5', { class: 'modal-title' }, `Thanks for requesting a ticket purchase! We will send an email to ${purchaseEmail.value} to complete the order form!`)));
    }
